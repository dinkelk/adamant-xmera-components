...
with Css_Weighted_Least_Squares_Output;
...
package body Css_Weighted_Least_Squares_Tests.Implementation is
   ...
   -- Check the estimate published by the most recent tick. The published record is
   -- unpacked first, so the comparisons run on native floats.
   procedure Assert_Latest_Estimate (
      ...
      T : Component.Css_Weighted_Least_Squares.Implementation.Tester.Instance_Access renames Self.Tester;
      Estimate : Css_Weighted_Least_Squares_Output.U;
   begin
      Natural_Assert.Eq (T.Data_Product_T_Recv_Sync_History.Get_Count, Tick_Number);
      Natural_Assert.Eq (T.Sun_Heading_Estimate_History.Get_Count, Tick_Number);
      Estimate := Css_Weighted_Least_Squares_Output.Unpack (T.Sun_Heading_Estimate_History.Get (Tick_Number));
      Packed_F32x3_U_Assert.Eq (Estimate.Sun_Heading_B, Heading, Epsilon => Epsilon);
      Packed_F32x3_U_Assert.Eq (Estimate.Omega_Bn_B, Omega, Epsilon => Omega_Epsilon);
      Unsigned_32_Assert.Eq (Estimate.Num_Css_Viewing_Sun, Count);
   ...
   procedure Assert_Latest_Residuals (Self : in out Instance; Tick_Number : in Natural; Residuals : in Packed_F32x8.U) is
      T : Component.Css_Weighted_Least_Squares.Implementation.Tester.Instance_Access renames Self.Tester;
      Estimate : constant Css_Weighted_Least_Squares_Output.U :=
         Css_Weighted_Least_Squares_Output.Unpack (T.Sun_Heading_Estimate_History.Get (Tick_Number));
   begin
      Packed_F32x8_U_Assert.Eq (Estimate.Post_Fit_Residuals, Residuals, Epsilon => Epsilon);
   ...
   begin
      ...
      Natural_Assert.Eq (T.Sun_Heading_Estimate_History.Get_Count, 0);
