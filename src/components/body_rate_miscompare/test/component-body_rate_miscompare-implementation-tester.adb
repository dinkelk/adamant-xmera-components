...
package body Component.Body_Rate_Miscompare.Implementation.Tester is
   ...
   begin
      ...
      if Length_To_Return = 0 then
         case Arg.Id is
            -- Length for Imu_Body:
            when 0 => Length_To_Return := Mimu_Majority_Vote_Output.Size_In_Bytes;
      ...
      if Return_Status = Data_Product_Enums.Fetch_Status.Success then
         case Arg.Id is
            -- Length for Imu_Body:
            when 0 =>
               Buffer_To_Return (Buffer_To_Return'First .. Buffer_To_Return'First + Mimu_Majority_Vote_Output.Size_In_Bytes - 1) :=
                  Mimu_Majority_Vote_Output.Serialization.To_Byte_Array (Self.Imu_Body);
            -- Length for Star_Tracker_Attitude:
            when 1 =>
