--------------------------------------------------------------------------------
-- Rate_Control Component Tester Body
--------------------------------------------------------------------------------

-- Includes:
with Parameter;

package body Component.Rate_Control.Implementation.Tester is
   ...
   begin
      ...
      Self.Torque_Command_History.Init (Depth => 100);
   end Init_Base;
   ...
   begin
      ...
      -- Data product histories:
      Self.Torque_Command_History.Destroy;
   end Final_Base;
   ...
   begin
      ...
      if Length_To_Return = 0 then
         case Arg.Id is
            -- Length for Attitude_Guidance:
            when 0 => Length_To_Return := Att_Guid.Size_In_Bytes;
            -- If ID can not be found, then return ID out of range error.
            when others =>
               if Return_Status = Data_Product_Enums.Fetch_Status.Success then
                  Return_Status := Data_Product_Enums.Fetch_Status.Id_Out_Of_Range;
      ...
      if Return_Status = Data_Product_Enums.Fetch_Status.Success then
         case Arg.Id is
            -- Length for Attitude_Guidance:
            when 0 =>
               Buffer_To_Return (Buffer_To_Return'First .. Buffer_To_Return'First + Att_Guid.Size_In_Bytes - 1) :=
                  Att_Guid.Serialization.To_Byte_Array (Self.Attitude_Guidance);
   ...
   -----------------------------------------------
   -- Description:
   --    Data products for the Rate Control component.
   -- Command torque vector in body frame components.
   overriding procedure Torque_Command (Self : in out Instance; Arg : in Packed_F32x3_Record.T) is
   begin
      -- Push the argument onto the test history for looking at later:
      Self.Torque_Command_History.Push (Arg);
   end Torque_Command;
...
end Component.Rate_Control.Implementation.Tester;
