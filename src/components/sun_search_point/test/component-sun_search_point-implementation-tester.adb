--------------------------------------------------------------------------------
-- Sun_Search_Point Component Tester Body
--------------------------------------------------------------------------------

-- Includes:
with Parameter;

package body Component.Sun_Search_Point.Implementation.Tester is
   ...
   begin
      ...
      if Length_To_Return = 0 then
         case Arg.Id is
            -- Length for Omega_Bn_B:
            when 0 => Length_To_Return := Packed_F32x3.Size_In_Bytes;
            -- Length for R_Hat_Sb_B:
            when 1 => Length_To_Return := Packed_F32x3.Size_In_Bytes;
            -- Length for Num_Css_Viewing_Sun:
            when 2 => Length_To_Return := Packed_U32.Size_In_Bytes;
            -- If ID can not be found, then return ID out of range error.
            when others =>
               if Return_Status = Data_Product_Enums.Fetch_Status.Success then
                  Return_Status := Data_Product_Enums.Fetch_Status.Id_Out_Of_Range;
         ...
         case Arg.Id is
            ...
            when 0 =>
               Buffer_To_Return (Buffer_To_Return'First .. Buffer_To_Return'First + Packed_F32x3.Size_In_Bytes - 1) :=
                  Packed_F32x3.Serialization.To_Byte_Array (Self.Omega_Bn_B);
            -- Length for R_Hat_Sb_B:
            when 1 =>
               Buffer_To_Return (Buffer_To_Return'First .. Buffer_To_Return'First + Packed_F32x3.Size_In_Bytes - 1) :=
                  Packed_F32x3.Serialization.To_Byte_Array (Self.R_Hat_Sb_B);
            -- Length for Num_Css_Viewing_Sun:
            when 2 =>
               Buffer_To_Return (Buffer_To_Return'First .. Buffer_To_Return'First + Packed_U32.Size_In_Bytes - 1) :=
                  Packed_U32.Serialization.To_Byte_Array (Self.Num_Css_Viewing_Sun);
   ...
   begin
      -- Push the argument onto the test history for looking at later:
      Self.Sun_Search_Status_History.Push (Arg);
   end Sun_Search_Status;
