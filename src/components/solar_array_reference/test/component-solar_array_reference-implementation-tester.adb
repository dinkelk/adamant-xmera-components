   ...
   begin
      ...
      if Length_To_Return = 0 then
         case Arg.Id is
            ...
            when 4 => Length_To_Return := Packed_F32.Size_In_Bytes;
            -- Length for Offset_Angle:
            when 5 => Length_To_Return := Packed_F32.Size_In_Bytes;
            -- If ID can not be found, then return ID out of range error.
            when others =>
               if Return_Status = Data_Product_Enums.Fetch_Status.Success then
                  Return_Status := Data_Product_Enums.Fetch_Status.Id_Out_Of_Range;
         ...
         case Arg.Id is
            ...
            when 4 =>
               Buffer_To_Return (Buffer_To_Return'First .. Buffer_To_Return'First + Packed_F32.Size_In_Bytes - 1) :=
                  Packed_F32.Serialization.To_Byte_Array (Self.Specified_Array_Angle);
            -- Length for Offset_Angle:
            when 5 =>
               Buffer_To_Return (Buffer_To_Return'First .. Buffer_To_Return'First + Packed_F32.Size_In_Bytes - 1) :=
                  Packed_F32.Serialization.To_Byte_Array (Self.Offset_Angle);
