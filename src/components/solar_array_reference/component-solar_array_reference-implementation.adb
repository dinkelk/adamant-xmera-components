...
package body Component.Solar_Array_Reference.Implementation is
   ...
   -- goes through here so the configuration is assembled in exactly one place. The
   -- parameters were checked at staging and the commanded values by the component
   -- that commands them, so a set the algorithm would reject is a defect.
   ...
   begin
      ...
      -- configuration to it, so the whole configuration is pushed at once.
      if Mode_Status = Success or else
         Specified_Angle_Status = Success or else
         Offset_Status = Success
      then
         Self.Commanded_Mode := To_C (Mode.Value);
      ...
      -- fields the shim needs. The current array angle only matters to the algorithm
      -- when the sun is nearly along the drive axis, and it is not wired here.
      -- TODO: remove the Theta argument once it is removed from the algorithm and its
      -- C shim; until then it is passed as zero.
