...
private
   ...
   type Instance is new Solar_Array_Reference.Base_Instance with record
      ...
      -- type so it is converted once, when commanded. The defaults keep the
      -- configuration valid for the generated parameter default check, which runs
      -- before Init.
      Commanded_Mode : Solar_Array_Tracking_Mode := Auto_Track;
      Commanded_Angle : Short_Float := 0.0;
      Commanded_Offset : Short_Float := 0.0;
   end record;
