...
private
   ...
   -- A duty cycle at or above Level_On latches the thruster on at the minimum fire
   -- time, and one at or below Level_Off latches it off, regardless of the previous
   -- state.
   ...
   -- Off-pulsing adds the maximum thrust to the requested force and clamps the sum
   -- at zero.
   overriding procedure Test_Off_Pulsing_Offset (Self : in out Instance);
   -- Each thruster carries its own hysteresis state, so the same intermediate force
   -- yields different on-times.
