--------------------------------------------------------------------------------
-- Sunline_Filter Component Tester Body
--------------------------------------------------------------------------------

-- Includes:
with Parameter;

package body Component.Sunline_Filter.Implementation.Tester is
   ...
   begin
      ...
      -- Event histories:
      Self.Diagnostics_Packet_Period_Set_History.Init (Depth => 100);
      Self.Invalid_Command_Received_History.Init (Depth => 100);
      -- Data product histories:
      Self.Sun_Direction_Estimate_History.Init (Depth => 100);
      -- Packet histories:
      Self.Filter_Diagnostics_History.Init (Depth => 100);
   end Init_Base;
   ...
   begin
      ...
      -- Event histories:
      Self.Diagnostics_Packet_Period_Set_History.Destroy;
      Self.Invalid_Command_Received_History.Destroy;
      -- Data product histories:
      Self.Sun_Direction_Estimate_History.Destroy;
      -- Packet histories:
      Self.Filter_Diagnostics_History.Destroy;
   end Final_Base;
         ...
         case Arg.Id is
            ...
            when 0 =>
               Buffer_To_Return (Buffer_To_Return'First .. Buffer_To_Return'First + Packed_F32x3.Size_In_Bytes - 1) :=
                  Packed_F32x3.Serialization.To_Byte_Array (Self.Body_Rate);
            -- Length for Css_Cosines:
            when 1 =>
               Buffer_To_Return (Buffer_To_Return'First .. Buffer_To_Return'First + Css_Sensor_Values.Size_In_Bytes - 1) :=
                  Css_Sensor_Values.Serialization.To_Byte_Array (Self.Css_Cosines);
   ...
   -- packet off.
   overriding procedure Diagnostics_Packet_Period_Set (Self : in out Instance; Arg : in Packed_U16.T) is
   begin
      -- Push the argument onto the test history for looking at later:
      Self.Diagnostics_Packet_Period_Set_History.Push (Arg);
   end Diagnostics_Packet_Period_Set;
   ...
   -- zero.
   overriding procedure Sun_Direction_Estimate (Self : in out Instance; Arg : in Nav_Att_Output.T) is
   begin
      -- Push the argument onto the test history for looking at later:
      Self.Sun_Direction_Estimate_History.Push (Arg);
   end Sun_Direction_Estimate;
   ...
   overriding procedure Filter_Diagnostics (Self : in out Instance; Arg : in Sunline_Filter_Output.T) is
   begin
      -- Push the argument onto the test history for looking at later:
      Self.Filter_Diagnostics_History.Push (Arg);
   end Filter_Diagnostics;
